...
with DAG(
    ...
    bronze_label_store = BashOperator(
        ...
        bash_command=(
            ...
            '--snapshotdate "{{ ds }}"'     # date will be changed accordingly when run
        ),
    )
    ...
    dep_check_source_data_bronze_2 >> bronze_table_2 >> silver_table_1 >> gold_feature_store    # chain multiple things into silver1
    dep_check_source_data_bronze_3 >> bronze_table_3 >> silver_table_2 >> gold_feature_store
    gold_feature_store >> feature_store_completed   # can repeat names, can bring stuff down since everything goes into gold_feature_store
    ...
    model_inference_start >> model_2_inference >> model_inference_completed     # split 1 thing into 2 different things
